    ...
    private void populateUI(Sandwich sandwich) {
        ...
        if (sandwich.getAlsoKnownAs().size() > 0) {
            ...
            for (String otherName : sandwich.getAlsoKnownAs()) {
                alsoKnown.append(otherName).append("\n");
            }
            alsoKnown.setLength(alsoKnown.length() - 1);
            alsoKnownAsView.setText(alsoKnown);
        } else {
            alsoKnownAsView.setVisibility(View.GONE);
            TextView alsoKnownLabel = findViewById(R.id.also_known_tv_label);
